...
def generate_launch_description():
    # Locate the vive_teleop.launch.py file
    ...
    only_right_arg = DeclareLaunchArgument(
        'only_right',
        default_value='true',
        description="If set, only the right controller will be active."
    )

    linear_scale_arg = DeclareLaunchArgument(
        'linear_scale',
        default_value='1.0',
        description='Scaling factor for controller translation.'
    )
    ...
    include_vive_teleop = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            'linear_scale': LaunchConfiguration('linear_scale') # Passed to vive_node
        }.items()
    )
