...
class CalibrationNode(Node):
    def __init__(self):
        ...
        # Exact path to your config file
        self.declare_parameter('config_path', '/ros2_ws/src/ros2_vive_controller/config/vive.params.yaml')
        ...
        # --- Publishers ---
        self.pub_marker = self.create_publisher(Marker, 'calibration_marker', 10)
    ...
    def publish_markers(self):
        ...
        box.color.b = 1.0; box.color.a = 0.3
    ...
    def save_and_exit(self):
        ...
        clean_pts = pts[(np.abs((pts - mean) / (std + 1e-9)) < 3.0).all(axis=1)]
        ...
        # Add padding (e.g., 5cm buffer is safer)
        padding = 0.05
        ...
        try:
            ...
            # Navigation to the correct ROS parameter node
            # Structure is: /** -> ros__parameters -> workspace
            if '/**' not in data: data['/**'] = {}
            if 'ros__parameters' not in data['/**']: data['/**']['ros__parameters'] = {}

            params = data['/**']['ros__parameters']
            if 'workspace' not in params: params['workspace'] = {}

            ws = params['workspace']
            # Update limits (Rounded to 3 decimals)
            ...
            self.get_logger().info(f"SUCCESS! Config updated: {self.config_path}")
